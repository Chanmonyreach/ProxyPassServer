...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    const BankHolder = 'N/A', BankNumber = 0, plateNumber = 'N/A';

    // Insert the user into the database
    const insertQuery = 'INSERT INTO users (username, email, plateNumber, password, BankHolder, BankNumber) VALUES (?, ?, ?, ?, ?, ?)';
    await db.promise().query(insertQuery, [username, email, plateNumber, hashedPassword, BankHolder, BankNumber]);
...
app.post('/stationData', (req, res) => {
  ...
  const { stationID, targetPower, chargePercent, cost, chargeProcess, chargeTime, location } = req.body;
  ...
  // If updating targetPower, chargePercent, cost, chargeTime, location
  if (targetPower && chargePercent && cost && chargeTime && location) {
    ...
    db.query(checkQuery, [stationID], (err, result) => {
      ...
      if (result.length > 0) {
        ...
        const updateQuery =
          'UPDATE stations SET targetPower = ?, chargePercent = ?, cost = ?, chargeTime = ?, location = ST_GeomFromText(?) WHERE stationID = ?';
        db.query(
          updateQuery,
          [targetPower, chargePercent, cost, chargeTime, formattedLocation, stationID],
          (err, updateResult) => {
            if (err) {
        ...
        const insertQuery =
          'INSERT INTO stations (stationID, targetPower, chargePercent, cost, payment, chargeProcess, chargeTime, location, connection) VALUES (?, ?, ?, ?, ?, ?, ?, ST_GeomFromText(?),?)';
        db.query(insertQuery, [stationID, targetPower, chargePercent, cost, payment, chargeProcess, chargeTime, formattedLocation, connection], (err, insertResult) => {
          if (err) {
...
app.get('/requestData', (req, res) => {
  ...
  if (userID) {
    const userQuery = 'SELECT username, sex, age, email, BankHolder, BankNumber, model, plateNumber, wallet, profileImg FROM users WHERE userID = ?';
  ...
  if (stationID) {
    const stationQuery =
      'SELECT stationID,LimitCurrent, targetPower, chargePercent, cost, payment, chargeProcess, chargeTime, location, connection FROM stations WHERE stationID = ?';
    ...
    db.query(stationQuery, [stationID], (err, stationResult) => {
      ...
      return res.json({
        ...
        chargePercent: stationData.chargePercent,
...
app.post('/updateUserData', async (req, res) => {
  const { userID, username, sex, age, email, model, plateNumber, BankHolder, BankNumber, profileImg } = req.body;

  // Validate required fields
  // if (!userID || !username || !sex || !age || !email || !model || !plateNumber || !BankHolder || !BankNumber) {
  ...
  db.query(checkQuery, [username, email, userID], (err, result) => {
    ...
    // Update user data
    const updateQuery = 'UPDATE users SET username = ?, sex = ?, age = ?, email = ?, Model = ?, plateNumber = ?, BankHolder = ?, BankNumber = ?, profileImg = ? WHERE userID = ?';
    db.query(updateQuery, [username, sex, age, email, model, plateNumber, BankHolder, BankNumber, profileImgBuffer || null, userID], (err, updateResult) => {
      if (err) {
...
app.post('/updatePassword', (req, res) => {
  ...
  db.query(selectQuery, [userID], (err, result) => {
    ...
    // Step 2: Compare old password using bcrypt
...
const resetStationData = async (stationID) => {
  const chargePercent = 0, cost = 0, payment = "N/A", Process = "N/A", duration = "00:00:00", connection = "Available";
  return new Promise((resolve, reject) => {
      db.query(
        'UPDATE stations SET chargePercent = ?, cost = ?, payment = ?, chargeProcess = ?, chargeTime = ?, connection = ? WHERE stationID = ?',
        [chargePercent, cost, payment, Process, duration, connection, stationID],
        (err, results) => {
          if (err) return reject(err);
          resolve(results);
      });
...
app.get('/checking', (req, res) => {
  ...
  if (!stationID && userID) {
    db.query('SELECT stationID FROM stations WHERE connection = ?', [userID], (err, results) => {
      ...
      if (results.length === 0) {
          // No stationID found
          return res.status(200).json({ connectionAvailable: false });
      }
  
      // stationID found
      const stationID = results[0].stationID;
      return res.status(200).json({ connectionAvailable: true, station: stationID });
  });
...
app.get('/payment', (req, res) => {
  ...
  db.query('SELECT connection FROM stations WHERE stationID = ?', [stationID], (err, status) => {
    ...
    db.query('SELECT wallet FROM users WHERE userID = ?', [userID], (err, users) => {
      ...
      db.query('SELECT cost, chargePercent FROM stations WHERE stationID = ?', [stationID], (err, stations) => {
        if (err) {
        ...
        let power = parseFloat(stations[0].chargePercent);
        ...
        if (money >= cost && money != 0) {
          ...
          db.query('UPDATE users SET wallet = ? WHERE userID = ?', [total, userID], (err, userUpdate) => {
            ...
            db.query('UPDATE stations SET payment = ?, connection = ? WHERE stationID = ?', [paymentStatus, connectionStatuse, stationID], (err, stationUpdate) => {
              ...
              const message = `The payment of $${cost} was successful. Thank you for choosing our service. We sincerely appreciate your trust and wish you a pleasant and safe journey ahead!`;
...
// Notify user of insufficient funds when no stationID is provided
app.get('/notify-insufficient-funds', (req, res) => {
  ...
  // const message = `We're sorry, but you don't have enough funds in your balance to complete this payment. Thank you for choosing our service. We sincerely appreciate your trust and wish you a pleasant and safe journey ahead!`;
  insertNotification(userID, message);
...
app.post('/topup', (req, res) => {
  ...
  // Step 1: Fetch user by ID
  const checkUserQuery = 'SELECT wallet FROM users WHERE userID = ?';
  ...
  db.query(checkUserQuery, [userID], (err, result) => {
    ...
    // Step 2: Get current wallet amount
    const currentWallet = parseFloat(result[0].wallet);
    const topupAmount = parseFloat(amount);
    const newWallet = currentWallet + topupAmount;

    // Step 3: Update wallet
    const updateWalletQuery = 'UPDATE users SET wallet = ? WHERE userID = ?';
    db.query(updateWalletQuery, [newWallet, userID], (err2, result2) => {
      if (err2) {
        console.error('Wallet update error:', err2);
        return res.status(500).json({ error: 'Failed to update wallet' });
      }

      // Build the success message with variable interpolation
      const message = `You have successfully added $${topupAmount.toFixed(2)} to your wallet. Your current balance is $${newWallet.toFixed(2)}. Thank you for using our services.`;

      // Optionally insert notification to DB or log
      insertNotification(userID, message);

      // Step 4: Return success response
      res.status(200).json({
        // message,
        previousWallet: currentWallet.toFixed(2),
...
app.post('/rate', (req, res) => {
  ...
  if (!userID || !star) {
    return res.status(400).json({ error: 'Missing userID or star' });
  }

  const checkUserQuery = 'SELECT userID FROM rating_system WHERE userID = ?';
  ...
  db.query(checkUserQuery, [userID], (err, results) => {
    ...
    if (results.length === 0) {
      ...
      if (comment) {
        insertQuery = 'INSERT INTO rating_system (userID, Star, Comment) VALUES (?, ?, ?)';
        values = [userID, star, comment];
      } else {
        insertQuery = 'INSERT INTO rating_system (userID, Star) VALUES (?, ?)';
        values = [userID, star];
      }
      ...
      if (comment) {
        updateQuery = 'UPDATE rating_system SET Star = ?, Comment = ? WHERE userID = ?';
        values = [star, comment, userID];
      } else {
        updateQuery = 'UPDATE rating_system SET Star = ? WHERE userID = ?';
        values = [star, userID];
      }
